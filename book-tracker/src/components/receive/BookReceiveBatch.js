...
function BookReceiveBatch({ id, numBooks, startBarcode, handleDeleteBatch, updateBatch }) {
    ...
    return (
        <div className="flex items-center justify-start space-x-5 w-full">
            <table className="table bg-amber-50 w-1/2">
                <thead>
                    <tr>
                        <th>Barcode</th>
                        <th>Τίτλος</th>
                ...
                <tbody>
                    {bookDetails.map((book, index) => (
                        <tr key={index} className={index % 2 === 0 ? 'bg-amber-100' : ''}>
                            <td>{book.barcode}</td>
                            <td>
                                <input
                                    type="text"
            ...
            <button
                className="btn btn-square btn-error mb-2"
